...
def load_other_data():
    ...
    # Toplu taşıma veri setini oku
    transport_df = pd.read_excel(
        'C:\\Users\\kdrmr\\TAKE_ME_TO_THE_HOUSE\\ilçe ve yıllara göre toplu taşıma seferleri.xlsx', header=4)
...
if section == 'Home':
    ...
    with col1:
        video_path = r"C:\Users\kdrmr\TAKE_ME_TO_THE_HOUSE\Swedish National Anthem - 'Du Gamla, Du Fria' (SV-EN).mp4"
        st.video(video_path)
...
#"C:/Users/oguzh/OneDrive/Masaüstü/TAKE_ME_TO_THE_HOUSE/cluster.html"
    ...
    tab1, tab2, tab3 = st.tabs(['County Markers', 'Heatmap', 'other chart'])
    with tab1:
        st.write('Interactive map of Sweden counties.')

        sweden_map = create_county_markers(df_coordinates_county['latitude'],
                                           df_coordinates_county['longitude'],
                                           df_coordinates_county['county'])
        folium_static(sweden_map, height=1200, width=1400)
    with tab2:
        heatmap = create_house_count_heatmap(df)
        folium_static(heatmap, height=1200, width=1400)

    with tab3:
        fig = px.scatter_mapbox(df_coordinates_county_price, lat="latitude", lon="longitude",
    ...
    st.image(image, caption="Green Space Distribution by County")
